---
name: fixed_amount_invoice_ocr
description: 支持定额发票识别。
version: 1.0.4
author: SCNet
license: MIT
tags:
...
---
# Sugon-Scnet 定额发票识别 OCR 技能

本技能封装了 Sugon-Scnet 定额发票识别 OCR 服务，通过单一接口即可调用 1 种识别能力，高效提取文字及票据信息。

## 功能特性

- **定额票据**：覆盖定额发票识别，自动提取关键字段。
...
| 参数名 | 类型 | 必填 | 描述 |
|--------|------|------|------|
| ocrType | string | 是 | 识别类型枚举。必须为以下之一：<br>• QUOTA_INVOICE（定额发票） |
| filePath | string | 是 | 待识别图片的本地绝对路径。支持 jpg、png、pdf 等常见格式。 |

### 命令行调用示例

```bash
   python .claude/skills/fixed_amount_invoice_ocr/scripts/main.py QUOTA_INVOICE /path/to/invoice.jpg
```
...
- “帮我识别这张定额发票，图片在 /Users/name/Downloads/invoice.jpg”
